...
function hrheads_parseXml(){
	...
	exit;
		...
		hrheads_log(0, '-' . '############## NEW REQUEST ##############');

		//Make sure that this is a POST request.
		if(strcasecmp($_SERVER['REQUEST_METHOD'], 'POST') != 0){
			//If it isn't, send back a 405 Method Not Allowed header.
			hrheads_log(0, '-' . 'Not a Post Request ['.$_SERVER['REQUEST_METHOD'].']');
		...
		foreach($xml->Fields->Field as $field):
			...
			if($field->attributes()->name == 'Category'):
				...
				foreach($field->Values->Value as $value):
					...
					if(!$parent_term_id):
					   ...
					   $parent_term_id = wp_insert_term( $term_name, 'jobs_category',	array(null, 'slug' => sanitize_title($term_name)) );
						if($parent_term_id) {
							$parent_term_id = $parent_term_id['term_id'];
						}

						hrheads_log(0, '-' . $term_name . ' : Not exists, created new term : ' . $parent_term_id);   
					else:  
					...
					foreach($value as $sub_value):					 
						...
						if($sub_value->attributes()->name == 'Sub Category'):
							...
							foreach($sub_value->Values->Value as $sub_sub_value):
								...
								if(!$child_term_id):
									$child_term_id = wp_insert_term( $child_term_name, 'jobs_category',	array('slug' => sanitize_title($child_term_name), 'parent' => $parent_term_id) );  
									hrheads_log(0, '--' . $child_term_name . ' : Not exists, created new child term : ' . (string)$child_term_id);   
								else:
									hrheads_log(0, '--' . $child_term_name . ' : Exists, do nothing : ' . (string)$child_term_id);   
								endif;
			...
			if($field->attributes()->name == 'Work Type'):
				...
				foreach($field->Values->Value as $value):
					...
					$parent_term_id = term_exists( (string)$term_name, 'jobs_category');

					if(!$parent_term_id):
						$parent_term_id = wp_insert_term( $term_name, 'jobs_category',	array('parent' => 2234,'slug' => sanitize_title($term_name)) ); 
						hrheads_log(0, '-' . $term_name . ' : Not exists, created new term : ' . $parent_term_id['term_id']);   
					else:  
						hrheads_log(0, '-' . $term_name . ' : Exists, do nothing : '. $parent_term_id);						  
					endif;
		...
		foreach($xml->Job as $job):
			...
			hrheads_log(0, '-----------------');  
			hrheads_log(0, '-----New Job-----');  
			hrheads_log(0, '-' . $job->Title);  
	...
	echo "\n\n\n Starting classification ".$classification->attributes()->name."\n";
						
				if($classification->attributes()->name == 'Category'):

					$t = (string)$classification;
					echo "Category: $t\n";
					$term = get_term_by('name', $t, 'jobs_category');
					hrheads_log(0, '- Found Category term: -' . $term->name );  
					echo "- Found term: - $term->name\n";
					if($term->term_id):
				...
				if($classification->attributes()->name == 'Sub Category'):
					...
					$t = (string)$classification;
					echo "Sub Category: $t\n";
					$term = get_term_by('name', (string) $t, 'jobs_category');
					if($term->term_id):
						hrheads_log(0, '- Found Sub Category term: -' . $term->name );  
						echo "- Found term: - $term->name\n";  
						$assignment_categories[] = $term->term_id;
					endif;
				endif;   
				...
				if($classification->attributes()->name == 'Location'):
					...
					$term = get_term_by('name', (string) $t, 'jobs_category');
					if($term->term_id):
						hrheads_log(0, '- Found Location term: -' . $term->name );  
						echo "- Found term: - $term->name\n";  
						$assignment_locations[] = $term->term_id;
					else:
						 hrheads_log(0, '- Not found term: -' . $t );  
					endif;							
				endif;											  
			...
			if($assignment_query->have_posts()):
				...
				while($assignment_query->have_posts()):
					...
					echo "Add terms to $assignment_post_id\n";
					print_r(array_merge($assignment_locations, $assignment_work_types));
					wp_set_post_terms($assignment_post_id, array_merge($assignment_locations, $assignment_work_types), 'jobs_category'); 

					wp_update_post($data);
	echo "Job id: $assignment_post_id\n";
...
function hrheads_log($type, $msg) {
	...
	#$log_file_name = $logs_path . 'log_debug.txt';	
