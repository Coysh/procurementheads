...
register_nav_menus(array(
	'top-bar-r-t'  => 'Top-bar Right Top',
	'top-bar-r-b'  => 'Top-bar Right Bottom',
	'footer-nav' => 'Footer',
));
 ...
 */
if ( ! function_exists( 'procheads_top_bar_menu' ) ) {
	function procheads_top_bar_menu($depth = 2) {
		wp_nav_menu( array(
			'container'      => false,
			'menu_class'     => 'desktop-menu',
			'items_wrap'     => '<ul id="%1$s" class="%2$s" data-dropdown-menu>%3$s</ul>',
			'theme_location' => 'top-bar-r-b',
			'depth'          => $depth,
			'fallback_cb'    => false,
...
if ( ! function_exists( 'procheads_top_bar_r_buttons' ) ) {
	function procheads_top_bar_r_buttons() {
		wp_nav_menu( array(
			'container'      => false,
			'menu_class'     => 'button-menu',
			'items_wrap'     => '<ul id="%1$s" class="%2$s">%3$s</ul>',
			'theme_location' => 'top-bar-r-t',
			'link_after'          => ' <span class="fa-stack fa-gray"><i class="fa fa-circle-thin fa-stack-2x"></i><i class="fa fa-angle-right fa-stack-1x"></i></span>',
			'depth'          => 1,
			'fallback_cb'    => false,
			'walker'         => new Foundationpress_Top_Bar_Walker(),
		));
...
if ( ! function_exists( 'procheads_footer' ) ) {
	function procheads_footer() {
		wp_nav_menu( array(
			'container'      => false,
 ...
 */
if ( ! function_exists( 'procheads_mobile_nav' ) ) {
	function procheads_mobile_nav() {
		wp_nav_menu( array(
			'container'      => false,
			'menu'           => __( 'mobile-nav', 'procheads' ),
...
*/
if ( ! function_exists( 'procheads_add_menuclass' ) ) {
	function procheads_add_menuclass( $ulclass ) {
		$find = array('/<a rel="button"/', '/<a title=".*?" rel="button"/');
		$replace = array('<a rel="button" class="button"', '<a rel="button" class="button"');

		return preg_replace( $find, $replace, $ulclass, 1 );
	}
	add_filter( 'wp_nav_menu','procheads_add_menuclass' );
}
...
if ( ! function_exists( 'procheads_breadcrumb' ) ) {
	function procheads_breadcrumb( $showhome = true, $separatorclass = false ) {
		...
		if ( ! is_front_page() ) {
			...
			if ( is_single() && ! is_attachment() ) {
				...
				// Paginated archives
				echo '<li class="current item-current-' . get_query_var('paged') . '">' . __('Page', 'procheads' ) . ' ' . get_query_var('paged') . '</li>';
